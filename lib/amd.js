! function (root) {
     ...
     * @param {*} mod 
     */
    function _requireModules(mod) {
        var path = _normalize(_base || location.href, mod),
            module = Cache.get(path);
        if (!module.loaded) {
            throw new Error('script not yet load');
        }
     ...
     * @param {*} modules 
     */
    function _makeModules(modules) {
        var mods = [];
        modules.forEach(function (mod) {
            mods.push(_requireModules(mod));
        });
        return mods;
    }
            ...
            make: function (baseUrl) {
                ...
                stack.forEach(function (o) {
                    var
                        ...
                        deps = o.d,
                        factory = o.f;
                    return makeRequire({ base: baseUrl })(deps, function () {
                        var loader = Cache.get(module);
                        !loader.loaded && loader.set(factory);
                    }, 0, true);
                });
                ...
                function _checkDeps() {
                    ...
                    if (!deps.length && !fired) {
                        ...
                        sync ?
                            succ() :
                            setTimeout(function () {
                                // require([], function() {}) , 执行回调
                                // _makeModules 分析依赖并把依赖作为参数传给factory回调
                                succ.apply(self, _makeModules(_deps));
                            }, 0);
                    }
                }
                _checkDeps();
            } else {
                // 这块的逻辑是为了factory里面执行require('xxx')
                var 
    ...
    var require = root.require = function () {
        ...
        // 收集define, 分析依赖关系并存入Cache里面
        _module.make(location.href);
        if (_base) {
